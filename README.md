...
- 每个普通直接轮次默认使用 `untrusted` 严格审批；已有空闲线程或已完成配置的新会话草稿可为
  下一次直接轮次临时开启 `on-request` 沙箱内自动运行。当前 sandbox 已允许的操作自动执行，
  sandbox 升级、受限网络和工作区外写入仍显示人工审批。Working 时开关禁用，轮次结束或启动
  失败后恢复严格默认，下一轮必须重新开启。已启动 turn 保留其启动时的策略；切换到其他会话
  再返回时仍显示该 turn 的锁定状态，不会继承或覆盖其他会话的下一轮选择。新线程创建、恢复、
  fork 和跨设备队列继续固定 `on-request`，steering 不携带策略；浏览器不能提交 `never`、
  `granular` 或 reviewer，也不依赖实验性设置 API。

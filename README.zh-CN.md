...
- 增量加载长会话历史，并对输出和超大轮次提供有界、可恢复的降级处理。
- 流式展示 Agent 消息、推理摘要、计划、命令输出、文件变更、MCP 调用和轮次差异。
- 展示带复制与换行控制的语法高亮代码块、统一/分栏差异，以及分组、可折叠且输出有界的工具活动。
- 在浏览器中审核命令与文件变更请求，并在本次浏览器会话中把审批理由保留在对应命令上。
- 回答 `request_user_input` 发出的结构化问题。
- 新建会话时确定绝对工作目录和沙箱，并在输入框旁选择下一轮使用的模型与推理强度；初始选择来自 Codex 的有效配置，其他选项来自 `model/list`。
- 中断正在执行的轮次。
- 支持桌面端与移动端响应式布局。
...
app-server 自带的 WebSocket 传输仍属于实验功能。Ask Codex 在自己的浏览器网关
...
cd ask-codex
npm install
ASK_CODEX_WORKSPACE=/项目的绝对路径 npm run dev
```
...
```bash
npm run build
ASK_CODEX_WORKSPACE=/项目的绝对路径 npm start
```

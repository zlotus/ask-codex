...
- 本次没有界面变更，因此未重复运行视觉检查。最近一次视觉检查于 2026-07-24 通过：
  `node scripts/visual-check.mjs` 针对 4173 端口上的生产构建使用确定性的桌面端和移动端
  夹具，覆盖新线程对话框、已配置模型选择、图片预览、工具分组和模拟审批理由；未发现
  水平溢出、控件重叠、富内容被裁切、控制台错误或页面错误。

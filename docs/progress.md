...
范围受控的线程生命周期管理已经实现：侧边栏通过 Active 和 Archived 视图分别列出
未归档和已归档线程，并通过桌面端右键、移动端长按或显式动作按钮打开同一个菜单；长按
本身不会执行删除。空闲线程可以归档、取消归档或经二次确认永久删除，正在执行轮次的
线程则受到保护。下一个里程碑回到面向项目的导航和只读发现功能，同时不改变人工审批或
远程访问的信任模型。
...
- Active/Archived 双视图和统一的线程动作菜单：桌面端支持右键，移动端支持 550 毫秒
  ...
  后确认删除。来自其他客户端的归档、恢复和删除通知会同步列表与当前选择；删除还会
  清理该线程在内存和 IndexedDB 中的浏览器本地图片预览。
...
- 流式显示消息、推理、计划、命令输出、文件变更、MCP 调用、网页搜索、轮次 diff，
  ...
  推理的条目显示活动动画。轮次 diff 明确呈现为位于轮次末尾的整轮变更汇总。流和消息
  大小均有明确边界。完成或重同步返回的非完整轮次快照不会清空已经流式物化的内容，只有
  明确的 `full` 快照可以替换条目。
...
- 侧边栏仍按单个线程平铺。尚无项目分组、Skills 视图、独立的跨线程 Activity 视图、
  用量面板、线程重命名或 fork 操作。
...
本轮已于 2026-07-31 使用 Node.js `v24.18.0`、npm `12.0.2` 和 Codex CLI
`0.146.0` 完成代码验证：

- `npm run typecheck` 通过。
- `npm run lint` 通过。
- `npm test` 通过：26 个文件、321 项测试。服务端测试在允许绑定回环套接字的环境中
  运行。
- `npm run build` 通过。
- `CHROME_BIN=/usr/bin/chromium ASK_CODEX_VISUAL_URL=http://127.0.0.1:4173 ASK_CODEX_VISUAL_OUTPUT=/tmp/ask-codex-visual-composer-reasoning npm run check:visual`
  ...
  模拟审批理由，以及连续推理分组、活动推理动画、轮次变更汇总和未确认发送恢复条；未
  发现水平溢出、控件重叠、富内容被裁切、控制台错误或页面错误。夹具在删除确认处取消，
  所有模拟 RPC 均被浏览器测试拦截，没有创建真实 Codex 轮次。

...
- 按 ADR 0025，当前受 Cloudflare Access 保护的移动部署刻意不发布 Web App Manifest、PWA
  安装图标或 Service Worker。真机加入这些资源后曾退化为账号首字母图标和带地址栏窗口，
  回退后机器人图标与无地址栏启动恢复；重新引入前必须通过受控真机验证，且不得弱化 Access。
...
- 2026-08-11 在仓库外运行不带 `--experimental` 的 `codex app-server generate-ts`，确认稳定
  ...
  `on-request + workspace-write` 列为 Auto 组合。实现只使用成熟的直接 `turn/start`，未调用
  实验性的设置 RPC 或其他实验性 API。队列此前还核对了稳定
  `TurnStartParams.clientUserMessageId` 与

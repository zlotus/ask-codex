...
范围受控的图片输入 MVP 和同源浏览器本地的重载预览已经实现，新线程也已恢复使用
app-server 的默认历史契约。已有分页线程仍保留超大轮次的条目级恢复。下一个里程碑
回到面向项目的导航和只读发现功能，同时不改变人工审批或远程访问的信任模型。
...
本轮已于 2026-07-25 使用 Node.js `v24.13.1`、npm `11.12.1` 和 Codex CLI
`0.145.0` 完成代码验证：

- `npm run typecheck` 通过。
- `npm run lint` 通过。
- `npm test` 通过：25 个文件、288 项测试。服务端测试在允许绑定回环套接字的环境中
  运行。
- `npm run build` 通过。
- `CHROME_BIN=/usr/bin/chromium-browser ASK_CODEX_VISUAL_URL=http://127.0.0.1:4174 npm run check:visual`
  针对当前生产构建通过。确定性的桌面端和 390×844 移动端夹具覆盖新线程对话框、已配置
  模型选择、图片草稿预览、发送后及页面重载后缩略图的加载、受控布局和新标签打开行为、
  工具分组和模拟审批理由；未发现水平溢出、控件重叠、富内容被裁切、控制台错误或页面
  错误，且未创建真实 Codex 轮次。

同日此前记录的协议验证仍适用于本轮未改变的协议路径：

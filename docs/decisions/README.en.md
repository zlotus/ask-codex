...
| [0002](0002-security-gateway-and-manual-approval.md) | Accepted; amended by 0020, with the relevant policy amended again by 0022 | Keep the gateway as a strict policy boundary and enforce manual approval. |
...
| [0022](0022-sandbox-aware-one-turn-auto-run.en.md) | Accepted | Use `untrusted` for ordinary direct turns and `on-request` for one-shot auto-run while retaining human review at the sandbox boundary. |

...
的宿主机。远程访问的便利性不得削弱默认人工审批，也不得把浏览器网关变成通用远程执行
API；用户可为已有空闲线程或已完成配置的新线程首轮明确启用一次免审批提示，但每个 turn
仍默认手动且必须逐轮选择，选择不得扩大沙箱权限或被持久化。
...
- 让有实际影响的操作保持可见且默认须经用户人工审批；每个一次性免提示轮次都必须由用户在
  空闲时明确启用，新线程首轮也遵循同一规则，并在结束后恢复默认值。
...
- 遵循有文档记录的 `codex app-server` 接口；协议细节变化时，从已安装的 CLI 生成
  bindings。
- 将 `AGENTS.md` 中的安全不变量视为规范要求。产品演进必须保留网关策略边界、默认人工
  审批、一次性免提示选择、失败时关闭的行为，以及彼此独立的远程访问关卡。

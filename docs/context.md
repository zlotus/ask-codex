...
的宿主机。远程访问的便利性不得削弱默认严格审批，也不得把浏览器网关变成通用远程执行
API。每个直接 turn 都由网关建立独立执行环境：默认手动模式使用 `untrusted + readOnly`，逐轮
显式开启的自动模式使用 `on-request + workspaceWrite`。自动 turn 仍把 sandbox 越界请求交给
用户；浏览器不能提供完整权限参数，选择也不得跨 turn 持久化。
...
- 让有实际影响的操作保持可见并默认采用手动执行环境；每个一次性自动 turn 都必须由用户在
  空闲时明确启用，新线程首轮也遵循同一规则，越界请求仍人工审批，并在结束后恢复默认值。
...
- 将 `AGENTS.md` 中的安全不变量视为规范要求。产品演进必须保留网关策略边界、逐 turn 独立的
  手动/自动执行环境、一次性自动选择、越界人工审批、失败时关闭的行为，以及彼此独立的远程
  访问关卡。

...
API；普通直接 turn 使用 `untrusted`，用户可为已有空闲线程或已完成配置的新线程首轮明确
启用一次 `on-request` 沙箱内自动运行。自动 turn 仍把 sandbox 越界请求交给用户，且选择必须
逐轮开启、不得扩大沙箱权限或被持久化。
...
- 让有实际影响的操作保持可见并默认采用严格审批；每个一次性沙箱内自动 turn 都必须由用户在
...
- 遵循有文档记录的 `codex app-server` 接口；协议细节变化时，从已安装的 CLI 生成
  bindings。
- 将 `AGENTS.md` 中的安全不变量视为规范要求。产品演进必须保留网关策略边界、默认严格
  审批、一次性沙箱内自动选择、越界人工审批、失败时关闭的行为，以及彼此独立的远程访问关卡。
